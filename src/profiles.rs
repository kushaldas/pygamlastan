...
use crate::security::{PyReplayCache, SecurityConfig};
...
impl AuthnRequestOptions {
    ...
    fn new(
        ...
        // Start from gamlastan's documented defaults, then overlay the fields the
        // caller provided. This keeps us forward-compatible: if gamlastan adds a
        // new option field with a sensible default, this binding still compiles
        // and behaves correctly without exposing the new knob.
        let mut o = gwb::AuthnRequestOptions::default();
        o.sp_entity_id = sp_entity_id;
        o.acs_url = acs_url;
        o.acs_index = acs_index;
        o.protocol_binding = protocol_binding;
        o.force_authn = force_authn;
        o.is_passive = is_passive;
        o.name_id_format = name_id_format;
        o.allow_create = allow_create;
        o.sp_name_qualifier = sp_name_qualifier;
        o.authn_context_class_refs = authn_context_class_refs.unwrap_or_default();
        o.authn_context_comparison = parse_comparison(authn_context_comparison)?;
        o.provider_name = provider_name;
        o.destination = destination;
        o.proxy_count = proxy_count;
        o.requester_ids = requester_ids.unwrap_or_default();
        o.attribute_consuming_service_index = attribute_consuming_service_index;
...
impl AuthnResult {
    ...
    fn attributes_dict(&self) -> std::collections::HashMap<String, Vec<String>> {
        ...
        for a in &self.inner.attributes {
            let vals: Vec<String> =
                a.values.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect();
    ...
    fn __repr__(&self) -> String {
        format!("AuthnResult(name_id={:?}, idp={:?})", self.inner.name_id, self.inner.idp_entity_id)
    }
}
...
fn process_response(
    ...
    let cache_ref: Option<&dyn gamlastan::security::ReplayCache> =
        py_cache.as_ref().map(|c| c as &dyn gamlastan::security::ReplayCache);

    let result = gsp::process_response_with_verified_signatures(
        &response.inner,
        &config.inner,
        cache_ref,
        sp_entity_id,
...
#[pyo3(signature = (
    ...
    expected_request_id=None, now=None, replay_cache=None,
))]
#[allow(clippy::too_many_arguments)]
...
#[pyclass(module = "pygamlastan.profiles", name = "ProcessedAuthnRequest", frozen)]
...
/// Process an incoming AuthnRequest (IdP side), optionally using SP metadata.
#[pyfunction]
#[pyo3(signature = (request, sp_metadata=None))]
...
impl ResponseOptions {
    ...
    fn new(
        ...
        let attributes = attributes.unwrap_or_default().into_iter().map(|a| a.inner).collect();
        ResponseOptions {
            inner: gwb::ResponseOptions {
                idp_entity_id,
                in_response_to,
...
fn create_unsolicited_response(
    ...
    let now = now.unwrap_or_else(Utc::now);
    let attrs: Vec<_> = attributes.unwrap_or_default().into_iter().map(|a| a.inner).collect();
...
impl InMemorySessionStore {
    #[new]
    fn new() -> Self {
        InMemorySessionStore { inner: GInMemorySessionStore::new() }
    }
